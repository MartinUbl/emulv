...
uint64_t ExampleDevice::GetStartAddress() {
    return start_address_;
}

uint64_t ExampleDevice::GetEndAddress() {
    return end_address_;
}

ExampleDevice::ExampleDevice(EventEmitter &emitter, uint64_t start_address, uint64_t end_address) : emitter_(emitter),
                                                                                                    start_address_(
                                                                                                            start_address),
                                                                                                    end_address_(
                                                                                                            end_address) {
